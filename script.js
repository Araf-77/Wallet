...
function renderDashboard(){
  ...
  const totalBalance=allInc-allExp;
  const debtNetTotal=state.debts.reduce((s,d)=>s+debtNet(d),0);
  const availableBalance=debtNetTotal>0?totalBalance-debtNetTotal:totalBalance+Math.abs(debtNetTotal);
  document.getElementById('stat-balance').textContent=fmt(totalBalance);
  const av=document.getElementById('stat-available'); if(av) av.textContent=fmt(availableBalance);
...
function clearData(){
  ...
  state.transactions=[];state.debts=[];saveState();renderDashboard();toast('🗑️ Data cleared');
}
